...
function StaffNoticesPage() {
  ...
  return (
    <DetailPageShell
      ...
      title="Notice Board"
      description="View official notices relevant to staff operations."
    >
      <ContentSection title="Notices" description="Latest active notices for staff.">
                ...
                <DataTableShell
                  ...
                  renderRow={(notice) => (
                    <tr key={notice.id} className="border-b border-slate-800/70 last:border-none">
                      ...
                      <td className="px-4 py-3">
                        <Button size="sm" variant="secondary" onClick={() => navigate(`/staff/notices/${notice.id}`)}>
                          View
                        </Button>
                      </td>
                    </tr>
              ...
              <div className="grid gap-3 md:hidden">
                {items.map((notice) => (
                  <article key={notice.id} className="rounded-2xl border border-slate-700/60 bg-bg-card/75 p-4">
                    ...
                    <Button className="mt-3" size="sm" variant="secondary" onClick={() => navigate(`/staff/notices/${notice.id}`)}>
                      Open Notice
                    </Button>
                  </article>
                ))}
